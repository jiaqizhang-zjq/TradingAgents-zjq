...
class TradingDatabase:
    ...
    def export_report_to_markdown(self, symbol: str, trade_date: str, 
        ...
        """将报告导出为Markdown文件（委托给db_exporters模块）"""
        from .db_exporters import export_report_to_markdown
        return export_report_to_markdown(self, symbol, trade_date, output_dir)
    
    def export_tool_calls_to_jsonl(self, symbol: str, trade_date: str,
                                    output_dir: str = "reports") -> str:
        """将工具调用记录导出为JSONL文件（委托给db_exporters模块）"""
        from .db_exporters import export_tool_calls_to_jsonl
